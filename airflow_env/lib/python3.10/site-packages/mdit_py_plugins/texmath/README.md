...
## How to Use the TeX Math Plugin with the Project

To use the TeX math plugin with your project, follow these steps:

1. Install the `markdown-it-texmath` plugin:
   ```bash
...
2. Import and use the plugin in your markdown-it setup:
   ...
   md.use(texmath.use(require('katex')));
   ```

3. Define TeX math equations in your markdown files:
   ```markdown
   $$
   \alpha = \frac{1}{2}
   $$
...
4. Render the markdown content with the TeX math equations:
   ```js
   var result = md.render('$$\alpha = \frac{1}{2}$$');
   console.log(result);
   ```

For more information on how to use the TeX math plugin, refer to the [official documentation](https://github.com/goessner/markdown-it-texmath).
